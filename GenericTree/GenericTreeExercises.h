...
static void treeFactory(GenericTree<int>& tree) {
  ...
  // ...
    ...
    // std::cout << "rootptr->data: " << rootptr->data << std::endl;
    rootptr->data = 4;
    auto node8 = rootptr->addChild(8);
    auto node15 = rootptr->addChild(15);
    
    auto node16 = node8->addChild(16);
    auto node23 = node8->addChild(23);
    
    auto node44 = node16->addChild(42);
}
...
std::vector<T> traverseLevels(GenericTree<T>& tree) {
  ...
  // ...
    std::deque<TreeNode*> myq = {rootNodePtr};
    // myq.push_back(rootNodePtr);
    while ( !myq.empty() ) {
        TreeNode* curnode = myq.front();
        myq.pop_front();
        results.push_back(curnode->data);
        for (TreeNode* childptr : curnode->childrenPtrs) {
            myq.push_back(childptr);
        }
    }
