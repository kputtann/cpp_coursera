...
void LinkedList<T>::insertOrdered(const T& newData) {
  ...
  // new node or on those existing nodes that are adjacent to the new node.
    ...
    if (size_ <= 1){
        ...
        head_->prev = newNode;
        head_ = newNode;
        // std::cout << "!!!!!!!!! After updating head_: head_->data, head->next->data = " << head_->data << "," << head_->next->data << std::endl;
        ...
        newNode->prev = tail_;
        tail_ = newNode;
        // std::cout << "!!!!!!!!! After updating tail_: tail_->data, tail_->prev->data = " << tail_->data << "," << tail_->prev->data << std::endl;
    } else {
        Node* cur = head_;
